...
void Compress(hashState *state)
{
  unsigned long long
    ...
    WT0, WT1, WT2, WT3, WT4;//, WT5; -- Unused var
...
int crypto_hash_echo512_generic_opt64(unsigned char *out,const unsigned char *in,unsigned long long inlen)
{
  if (Hash(CRYPTO_BYTES * 8,(BitSequence *)in,inlen * 8,out) == SUCCESS) return 0;
  return -1;
}
